...
// Add at the start of the file to enable observation
extension NSNotification.Name {
    static let dismissEmailVerification = NSNotification.Name("DismissEmailVerification")
}
...
class SignUpViewModel: ObservableObject {
    ...
    @Published var userToVerify: User? = nil
    ...
    func onEmailVerified() {
        ...
        guard let user = userToVerify else {
            showError(message: "Unable to complete signup process. Please try again.")
            return
        }
        ...
        Task {
            do {
                // Call the method to complete user registration in database
                let verifiedUser = try await supabaseManager.completeUserRegistration(user: user)
                
                // Now log in the user with the verified account
                await MainActor.run {
                    self.isLoading = false
                    signUpSuccess(with: verifiedUser)
                }
            } catch {
                await MainActor.run {
                    self.isLoading = false
                    self.showError(message: "Failed to complete registration: \(error.localizedDescription)")
                }
            }
        }
    }
